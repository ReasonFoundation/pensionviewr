...
filterData <- function(Data, fy, source = FALSE){
  ...
  Data <- Data %>% select(year, plan_name = display_name, 
                          ...
                          admin_exp = administrative_expense_dollar, cost_structure, asset_valuation_method_for_gasb_reporting,
                          payroll = covered_payroll_dollar, ee_contribution = employee_contribution_dollar, 
                          ee_nc_pct = employee_normal_cost_percentage, er_contribution = employer_contribution_regular_dollar, 
